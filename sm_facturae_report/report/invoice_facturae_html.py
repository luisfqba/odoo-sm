...
class invoice_facturae_html(report_sxw.rml_parse):
    ...
    def _get_sale_order(self, sale_ids):
        #pool = pooler.get_pool(self.cr.dbname)
        ...
        if sale_ids:
            res.update({
                'sucursal' : sale_ids[0].partner_id.name,
                'incoterm' : sale_ids[0].incoterm.code,
            })
        
        return res
        

webkit_report.WebKitParser('report.account.invoice.sm.facturae.webkit',
            'account.invoice',
            'addons/sm_facturae_report/report/invoice_facturae_html.mako',
